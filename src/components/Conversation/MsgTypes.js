...
const DocMsg = ({ el, menu }) => {
    ...
    const messageUserId = el.baseUser?.id;
    const channelId = useSelector(state => state.chat.selectedChannel.channelId);

    let attachments;
    if (typeof el.attachments === 'string') {
        attachments = JSON.parse(el.attachments)
    }

    console.log("Adding Doc message", el);

    const incoming = currentUser?.id !== messageUserId;
    console.log("Incoming", incoming, currentUser?.id, messageUserId);

    const onDownload = () => {
        console.log("Downlaod clicked", attachments?.[0]?.id)
            ...
            <Box p={1.5} sx={{
                ...
                <Stack spacing={2}>
                    <Stack p={2} spacing={3} direction='row' alignItems='center'
                        ...
                        <IconButton>
                            <DownloadSimple onClick={onDownload} />
                        </IconButton>
                    </Stack>
                    <Typography variant='body2' sx={{ color: incoming ? theme.palette.text : '#fff' }} >
                        {el.content}
                    </Typography>
                </Stack>
            </Box>
...
const TextMsg = ({ el, menu }) => {
    ...
    console.log("Adding Text message", el);
